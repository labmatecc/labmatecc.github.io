...
let
	url = "https://github.com/labmatecc/labmatecc.github.io/blob/main/Im%C3%A1genes/cuerpos1.png?raw=true"
...
begin
	url = "https://github.com/labmatecc/labmatecc.github.io/blob/main/Im%C3%A1genes/cuerpos2.png?raw=true"
	fname = download(url) #bajamos la imagen a la máquina local
	imag = load(fname) #declaramos la variable "imag"
end
...
md"""Grafiquemos la trayectoria de algunos planetas alrededor del Sol."""

# ╔═╡ eca2ca57-627e-4cfe-8ca6-50874e786220
begin
	p = 8955.586  # Parámetro
	e₁ = 0.093 # Excentricidad de Mercurio
	e₂ = 0.007 #Excentricidad de Venus
	e₃ = 0.017  # Excentricidad de Tierra
	...
	x_M = [distancia_al_sol(θ, p, e₁) * cos(θ) for θ in θ_values]
	y_M = [distancia_al_sol(θ, p, e₁) * sin(θ) for θ in θ_values]
	x_V = [distancia_al_sol(θ, p, e₂) * cos(θ) for θ in θ_values]
	y_V = [distancia_al_sol(θ, p, e₂) * sin(θ) for θ in θ_values]
	x_T = [distancia_al_sol(θ, p, e₃) * cos(θ) for θ in θ_values]
	y_T = [distancia_al_sol(θ, p, e₃) * sin(θ) for θ in θ_values]

	plot(x_V, y_V, aspect_ratio=:equal, xlabel="Distancia (millones de km)", ylabel="Distancia (millones de km)", label="Trayectoria Venus")
	plot!(x_T, y_T, aspect_ratio=:equal, label="Trayectoria Tierra")
	plot!(x_M, y_M, aspect_ratio=:equal, label="Trayectoria Marte")
	
	title!("Órbitas alrededor del Sol")

	scatter!([0], [0], markersize=5, label="Sol", color=:yellow)
end
