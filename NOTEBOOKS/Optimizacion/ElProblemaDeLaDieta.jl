...
*“Es una obligación eterna hacia el ser humano no dejarlo sufrir de hambre cuando uno tiene la oportunidad de acudir en su ayuda.”* 
– Simone Weil

[La situación del hambre en Colombia](https://www.elespectador.com/salud/la-situacion-del-hambre-en-colombia-si-es-critica-bancos-de-alimentos/)

La dieta de Stigler es un problema de optimización nombrado en honor a [George Stigler](https://es.wikipedia.org/wiki/George_Stigler), un laureado con el Premio Nobel de economía en 1982, quien planteó el siguiente problema:

*Para un hombre moderadamente activo que pesa 154 libras, ¿cuánto de cada uno de los 77 alimentos debe comer diariamente para que la ingesta de nueve nutrientes sea al menos igual a las recomendaciones dietéticas sugeridas por el Consejo Nacional de Investigación en 1943, con el costo de la dieta siendo mínimo?*
[El costo de subsistencia](https://math.berkeley.edu/~mgu/MA170/Diet.pdf)"""

# ╔═╡ 759e2e83-90f8-41b1-b413-720e61a496bd
md"""# El problema de la dieta

El objetivo del **problema de la dieta** es seleccionar *cantidades de diferentes tipos de alimentos* que **satisfagan** un conjunto de **requisitos nutricionales diarios al menor costo posible**.

## ¡Diferentes alimentos, diferentes nutrientes!

Cada tipo de alimento tiene una contribución nutricional y un precio por unidad (una unidad puede ser un gramo (gr)). Por ejemplo, 100 gr de arroz aportan 120 calorías, 0.28 gr de grasa, 28.17 gr de carbohidratos y 2.69 gr de proteínas, mientras que 100 gr de carne aportan 287 calorías, 19.29 gr de grasa, 0 gr de carbohidratos y 26.41 gr de proteínas. (Esta información se puede obtener fácilmente de un [sitio de nutrición](https://www.fatsecret.com/calories-nutrition/generic/meat?portionid=50026&portionamount=100.000)). Como es lógico, diferentes alimentos también tienen diferentes precios. Por ejemplo, en Colombia, 1000 gr de arroz cuestan 3,590 pesos colombianos (COP), mientras que 500 gr de carne cuestan 11,840 COP.
...
donde $c_1$ y $c_2$ son los costos de arroz y carne por gramo, respectivamente. La función objetivo $f_0(x_1,x_2)$ proporciona el costo de diferentes dietas al ponderar linealmente la cantidad seleccionada de alimentos por sus respectivos costos unitarios. Al evaluar esta función, podemos medir cuantitativamente el costo de cualquier dieta (ver este [calculador de dieta en línea](https://docs.google.com/spreadsheets/d/1TzIYFFXFHBOLXQzfLVKG7W2wrrd9mdjRo-6xgGmtYGY/edit?usp=sharing)). Por ejemplo, $f_0(200,216) = 5,833$ y $f_0(305,200) = 5,831$. Una dieta compuesta por $(x_1,x_2)=(200,200)$ costará $f_0(200,200) = 5,454$ COP.
"""

# ╔═╡ 648b2d01-c8a9-4012-a17e-1b81e7c7e116
md"""La **función objetivo** $f_0$ en el problema de la dieta se puede visualizar como una función que mapea desde el espacio de todas las dietas posibles ($\mathbf{R}^2$) al costo ($\mathbf{R}$), es decir, $f_0: \mathbf{R}^2 \rightarrow \mathbf{R}$:
"""
...
plot(0:500, 0:500, (x, y) -> c[1]*x + c[2]*y,
    xlabel = "rice (gr)", ylabel = "meat (gr)", zlabel = "diet cost",
    title = "Objective Function: Diet Cost", st = :surface)

end

# ╔═╡ 86b63a27-7af7-41b3-8238-12f5bb98701b
md"""En este caso, la función de costo vinculada al problema de la dieta está modelada por una función lineal. Vale la pena recordar que este es un modelo del proceso relacionado con el costo de una dieta. Tal vez el costo pueda diferir si compras más alimentos en grandes superficies, pero debemos destacar que estamos modelando; por lo tanto, la suposición lineal implícita en la función de costo ya no es válida. Sin embargo, debemos recordar que *modelar simplifica el mundo para entenderlo mejor*.
"""

# ╔═╡ b041cc88-49c8-4cc7-b748-edadee7aa81a
md"""# No puedes comer cantidades negativas de comida = ¿definiendo restricciones?
...
Supongamos que eres un atleta competitivo, por ejemplo, un ciclista de carreras. La cantidad de calorías consumidas por estos atletas en una etapa montañosa es de ¡alrededor de 7,000 calorías! [ref](https://www.cyclingweekly.com/news/racing/tour-de-france/this-is-what-you-have-to-eat-to-compete-in-the-tour-de-france-182775). Por lo tanto, si queremos usar nuestro modelo, deberíamos cambiar la primera restricción a:
...
md"""Esa es la razón por la que los atletas comen mucho ([ref](https://bleacherreport.com/articles/1932914-athletes-who-ate-insane-amounts-of-food)).

Por lo tanto, la próxima vez que alguien critique la forma en que comes, puedes decir: *'No estoy comiendo mucho, solo estoy garantizando que la región que acota el espacio de mi dieta factible no sea vacía'* :).
"""

# ╔═╡ 12696be3-1dbc-4231-a1c3-864389245786
md"""# Finalmente, el problema de optimización matemática
...
Minimiza el valor 3.59 x₁ + 23.68 x₂ entre todos los vectores (x₁, x₂) ∈ ℝ²,
satisfaciendo las siguientes restricciones:
  
  1.2    x₁ + 2.87   x₂ ≥ 1000
  0.0028 x₁ + 0.1929 x₂ ≥ 44
  0.2817 x₁ + 0      x₂ ≥ 0
  0.0269 x₁ + 0.2641 x₂ ≥ 35
  x₂ + x₁ ≤ 2000
  x₁ ≥ 0
  x₂ ≥ 0
...
md"""# Hacia una solución numérica del problema de optimización lineal

El hecho de que el problema sea lineal nos ayudará mucho a encontrar una solución. Las funciones lineales pueden visualizarse fácilmente.

## ¿Qué dietas se pueden construir usando un presupuesto fijo? = Líneas de nivel de la función objetivo
...
Después de la pandemia de COVID-19, la [pobreza extrema aumentó en Colombia](https://www.elespectador.com/economia/macroeconomia/la-pobreza-en-colombia-en-medio-de-la-pandemia/). El Banco Mundial define la pobreza extrema como personas que viven con $1$ dólar o menos al día. Podemos preguntarnos qué dietas se pueden proponer con un presupuesto de $1$ USD (aproximadamente $4000$ COP) por día.
...
md"""## Buscando soluciones al problema: De puntos infinitos a líneas infinitas
...
md"""## Buscando soluciones: De líneas infinitas a un conjunto finito de puntos
...
md"""# Finalmente, calcular la solución

Utilizando los puntos que constituyen la Envoltura Convexa de la región factible y la función objetivo, podemos encontrar fácilmente la solución a nuestro problema, e incluso preguntar si hay soluciones equivalentes.
"""
...
# ╔═╡ e5b7af6f-e040-4cfe-ab42-65f14106c182
md"""# Comentarios Finales

Después de que George Stigler formuló por primera vez el modelo, George Danzing lo resolvió más tarde utilizando su novedoso algoritmo Simplex. El algoritmo simplex "salta" entre esquinas utilizando nociones de soluciones base, simplex y pivoteo (la misma operación que se usa en la reducción gaussiana de sistemas lineales). El problema de la dieta fue resuelto en 1947, siendo considerado (al menos para mí) el inicio del área de optimización computacional.

En 2014, el chef de Google Anthony Marco ideó una receta utilizando una lista similar de ingredientes (con hígado de ternera en lugar de leche evaporada), llamada "Foie Linéaire à la Stigler"; un empleado de Google la describió como "deliciosa" [ref](http://googleresearch.blogspot.com/2014/09/sudoku-linear-optimization-and-ten-cent.html).

En Colombia, tengo informes incidentales de que estrategias similares fueron utilizadas en los años ochenta para planificar la dieta del equipo de ciclismo 'Café de Colombia', el primer equipo profesional que participó en el Tour de Francia. Recientemente, se utilizó un modelo similar para estudiar la dieta de los niños en Bogotá (Colombia) con conclusiones preocupantes [ref](https://www.ingentaconnect.com/content/wk/mpg/2017/00000065/00000006/art00019), por ejemplo, "las ingestas dietéticas recomendadas de tiamina para estos niños pequeños no se pudieron lograr con los alimentos locales tal como se consumen", y "Ninguna de las recomendaciones alimentarias probadas aseguró la adecuación dietética a nivel poblacional para tiamina, niacina y hierro a menos que se recomendara un alimento infantil fortificado".

El gobierno colombiano tiene un programa de nutrición infantil llamado [*Programa de Alimentación Escolar*](https://www.mineducacion.gov.co/1621/w3-printer-235135.html#:~:text=%C2%BFQu%C3%A9%20es%20el%20PAE%3F,del%20Sistema%20General%20de%20Participaciones.) que actualmente juega un papel significativo en sostener la ingesta nutricional de las poblaciones vulnerables después del COVID-19. Sin embargo, no estoy seguro de si conocen la posibilidad de optimizar sus dietas.
"""
