...
# ╔═╡ e4e0743d-8cc4-4271-9369-023b902890bd
begin
	using Plots
	using Optim
	using JSServe
...
# ╔═╡ 05ff99df-69a3-417a-b69c-ca0151d03763
PlutoUI.TableOfContents(title="Optimización", aside=true)

# ╔═╡ ca58fb66-50e4-4041-8ceb-88eb9f02138a
html"""
...
# ╔═╡ 2625b3cb-6318-4d65-a4be-068ee0565195
md"""## Ejemplo 1: Minimización de una función cuadrática $f(x) = (x - 3)^2 + 4$"""

# ╔═╡ 2da57135-2ab2-40b4-aa50-a97dc9f78576
begin
	f(x) = (x - 3)^2 + 4
	result = Optim.optimize(f, -10.0, 10.0) # Minimización en el intervalo [-10, 10]
	println("Resultado: x óptimo = $(Optim.minimizer(result))")
end

# ╔═╡ 68259488-fdb7-47ab-a086-a180587b03bf
begin
	x_vals = -10:0.1:10
	y_vals = f.(x_vals)
	plot(x_vals, y_vals, label="f(x) = (x - 3)^2 + 4", xlabel="x", ylabel="f(x)", 	legend=:topright)
	scatter!([Optim.minimizer(result)], [f(Optim.minimizer(result))], label="Minimizador", color=:red)
end

# ╔═╡ 9fecaef0-56a3-4030-af2b-c32e91c97646
md""" ## Ejemplo 2: Minimización de una función no lineal $f(x) = sin(x) + 0.5x^2$"""

# ╔═╡ aaaba0d0-76c3-4fcf-831f-02e2d96b3361
begin
	g(x) = sin(x) + 0.5x^2
	result2 = Optim.optimize(g, -5.0, 5.0)
	println("Resultado: x óptimo = $(Optim.minimizer(result2))")
end

# ╔═╡ edda6584-16a1-4bce-82c7-dc1ea38918d5
begin
	x_vals2 = -5:0.1:5
	y_vals2 = g.(x_vals2)
	plot(x_vals2, y_vals2, label="f(x) = sin(x) + 0.5x^2", xlabel="x", ylabel="f(x)", legend=:topright)
	scatter!([Optim.minimizer(result2)], [g(Optim.minimizer(result2))], label="Minimizador", color=:blue)
end

# ╔═╡ fc9d4514-d87d-41f7-afc8-66ddae0c48d6
md""" ## Ejemplo 3: Minimización de la función de Himmelblau"""

# ╔═╡ c78662c5-7cf4-424c-966c-6c081f9aece3
begin
	himmelblau(x) = (x[1]^2 + x[2] - 11)^2 + (x[1] + x[2]^2 - 7)^2
	result3 = Optim.optimize(himmelblau, [-6.0, -6.0])
	println("Resultado: x óptimo = $(Optim.minimizer(result3))")
end

# ╔═╡ 1333aa18-1d74-4c59-8036-3efcf90ccc96
begin
	x = -6:0.1:6
	y = -6:0.1:6
	z = [himmelblau([i, j]) for i in x, j in y]
	plot(x, y, z, st=:surface, xlabel="x", ylabel="y", zlabel="h(x, y)", title="Función de Himmelblau")
	scatter!([Optim.minimizer(result3)[1]], [Optim.minimizer(result3)[2]], [himmelblau(Optim.minimizer(result3))], color=:red, label="Punto óptimo")
end

# ╔═╡ 21d4bdad-edaf-4288-b61b-7d22884e81c2
    html"""
    <h2>Ejercicio</h2>
    <p>Prueba encontrar el máximo de la siguiente función:</p>
    <pre>f(x) = -x^4 + 2x^3 - x^2</pre>
    <p>Encuentra el valor máximo utilizando el intervalo [-2, 2].</p>
    <p>Consejo: usa <code>optimize</code> con una función negativa para maximizar.</p>
    """
...
manifest_format = "2.0"
project_hash = "cf5dce9d5368fc310b8b7bd39dc3da744c26bc53"
