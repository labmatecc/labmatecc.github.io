...
La notación expfp$(k)$ representa el exponente de punto flotante de orden $k$.
"""
...
begin
	...
	println("signo=",xeps[1],"\t exp=",xeps[2:9],"\t significando=",xeps[19:32])
	xmin=bitstring(floatmin(Float32))
	println("signo=",xmin[1],"\t exp=",xmin[2:9],"\t significando=",xmin[19:32])
	xmax=bitstring(floatmax(Float32))
	println("signo=",xmax[1],"\t exp=",xmax[2:9],"\t significando=",xmax[19:32])
...
begin
	...
	println("Después de ",N, " sumas(Float64) de 1/10, \nel resultado es ",suma64)
	x=1/10
	p=N*x
...
begin
	...
	println("Después de ",N, " sumas(Float32) de 1/10, el resultado es ",suma32)
	f32=Float32(1/10)
	p32=N*f32
...
begin
	xx=3E-324
	yy=2E-324
	print("x=",xx," es un número de máquina. Pero y=2E-324 es aproximado  a  y=\t",yy, ".\n")
	print("Al dividir x=3E-324 por dos tenemos x/2=\t",xx/2,".\n")
	print("Note también que 1/x=\t",1/xx,".")
end
...
begin
	...
	print("x=",xp," es un número de máquina pero 'y=1.8E+308' genera desbordamiento.\n")
end
...
Se presentan algunas expresiones indeterminadas con las que se debe tener cuidado a la hora de calcular con punto flotante."""
...
Un ejemplo de pérdida de dígitos (o cancelación) es el siguiente. """
...
**PROBLEMA (Sullivan) 3.** Con $x_0=1/10$, considere la sucesión,  

$x_{n+1}=\left\{\begin{array}{ll}
2x_n & \mbox{ si } x_n\in \left[0,\frac{1}{2}\right] \\
2x_n-1 &  \mbox{ si } x_n\in \left(\frac{1}{2},1\right].
\end{array}\right.$
para $n=1,2,\dots$. Calcule $x_1,x_2,x_3,\dots,x_{50}$ de forma análitica y después usando un código en python. Compare y explique.

Repetir pero ahora iniciando con $x_0=1/8$. Compare y explique. 

**PROBLEMA 4.** Explique la falla del Ariane y del American Patriot. Muestre cálculos numéricos para soportar su explicación. 
...
**PROBLEMA (Stoer) 6.** Ensaye calcular las funciones $f(x)=\sqrt{x^2+1}-1$, y $g(x)=\frac{x^2}{\sqrt{x^2+1}+1}$.
...
**PROBLEMA (Stewart) 8.** Repita el experimento arriba con la recurrencia $x_1=\frac{1}{3}, x_2=\frac{1}{12}$,  $x_{k+1}=2.25x_k-0.5x_{k-1}$.
...
**PROBLEMA 10.** Repita el experimento arriba con $P(x)=(x-1)^7$. Agrege a la figura el siguiente método 
...
manifest_format = "2.0"
project_hash = "480673a590360d368e9a74813b1ae9d76bcfe085"
...
git-tree-sha1 = "8e7bd2d2fab15dbbef7c97d8e77db58d7db1c1eb"
...
git-tree-sha1 = "a7ca655bef8d9ef4aa20c7c6e2804d53cf05aaa5"
